...
class HomeHeadView: UBaseCollectionReusableView {
    ...
    @IBAction func ButSpellGroupClick(_ sender: UIButton) {
        let sepllgroup = SpellGroupViewController()
    
        self.viewController()?.navigationController?.pushViewController(sepllgroup, animated: true)
